...
void print_help(const char *prog_name) {
  ...
  printf("Benchmark movebox with single or multi-threaded "
         "execution.\n\n");
  printf("Options:\n");
...
#pragma omp parallel for
      for (int i = 0; i < ITERATIONS; ++i) {
        for (int t = 0; t < TICKS_PER_ITERATION; ++t) {
          mvec2 Pos = s_MapTest2.m_vStates[0][t].m_Pos;
          mvec2 Vel = s_MapTest2.m_vStates[0][t].m_Vel;
          bool Grounded = 0;
  ...
  printf("movebox calls (mean ± σ):\t\t%s ± %s calls/s\n", aBuf, aBuff);
