...
static int get_pure_map_index(SCollision *pCollision, vec2 Pos) {
...
static int move_restrictions(int Direction, int Tile, int Flags) {
...
static void ThroughOffset(vec2 Pos0, vec2 Pos1, int *pOffsetX, int *pOffsetY) {
...
static bool is_through(SCollision *pCollision, int x, int y, int OffsetX,
...
static bool is_hook_blocker(SCollision *pCollision, int x, int y, vec2 Pos0,
                            vec2 Pos1) {
  int pos = get_pure_map_index(pCollision, vec2(x, y));
  unsigned char *pTileIdx = pCollision->m_GameLayer.m_pData;
